...
function formatPrice(value: number, currency: Currency, locale: string): string {
  // Whole zł; converted currencies show no decimals for amounts >= 100, two decimals below.
  const minimumFractionDigits = currency === 'PLN' ? 0 : value >= 100 ? 0 : 2
  ...
  try {
    ...
    return `${Math.round(value)} ${SYMBOLS[currency]}`
  }
}
...
export const TicketPricesClient: React.FC<Props> = ({
  ...
  const updatedLabel = useMemo(() => {
    ...
    try {
      return new Intl.DateTimeFormat(locale, { dateStyle: 'medium' }).format(new Date(updatedAt))
    } catch {
  ...
  const renderPrice = (row: Row) => {
    ...
    const main = convert(row.pricePLN, currency, rates)
    const mainStr = formatPrice(main, currency, locale)
    if (row.priceMaxPLN != null) {
      const max = convert(row.priceMaxPLN, currency, rates)
      return `${mainStr} – ${formatPrice(max, currency, locale)}`
    }
    if (row.pricePLNDiscount != null) {
      ...
      return (
        <>
          {mainStr}
          <span className="ml-2 text-sm opacity-70">({formatPrice(disc, currency, locale)}*)</span>
        </>
      )
    }
    return mainStr
          ...
          <div className="flex flex-wrap items-center gap-3 mb-6 text-sm">
            ...
            {currency !== 'PLN' && (
              <span className="opacity-70">
                1 zł ≈ {rates[currency].toFixed(currency === 'GBP' ? 4 : 3)} {SYMBOLS[currency]}
              </span>
            )}
            {updatedLabel && (
            ...
            {sections?.map((section, idx) => (
              ...
              >
                ...
                {hasRichTextContent(section.intro) && (
                  <RichText className="mb-4 text-base" content={section.intro} enableGutter={false} />
                )}
                            ...
                            >
                              ...
                              {section.columnHeaders!.map((_, cIdx) => {
                                ...
                                return (
                                  ...
                                  >
                                    {cell != null
                                      ? formatPrice(convert(cell.value, currency, rates), currency, locale)
                                      : '—'}
                                  </td>
                                )
                              })}
