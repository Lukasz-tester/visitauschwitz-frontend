...
export const MobileNavCaller: React.FC<{
  ...
  return (
    <div>
      <button
        className={`z-50 top-0 right-0 ease-in-out duration-1000 ${modalOpen ? 'w-16 h-16 opacity-85 hover:opacity-100 transition-opacity' : 'bg-background/70 md:hover:bg-card-foreground lg:bottom-0 right-0 rounded-bl-3xl w-16 h-16'} flex items-center justify-center fixed dark:text-white/80 text-3xl`}
      ...
      />
      {/* Nav panel — sibling of backdrop so it gets its own z-40 stacking context */}
      <div
        className={`pt-2 h-screen flex flex-col max-w-[500px] sm:max-w-[350px] fixed top-0 right-0 z-40 bg-card md:bg-card/95 transition-transform duration-300 ease-in-out ${modalOpen ? 'translate-x-0' : 'translate-x-full pointer-events-none'}`}
      >
